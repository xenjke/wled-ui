...
export function boardsReducer(state: BoardsState, action: BoardsAction): BoardsState {
  switch (action.type) {
    case 'LOAD_SAVED':
      return { ...state, boards: action.boards.sort(sortBoards), lastRefresh: action.lastRefresh };
    case 'DISCOVERY_START':
      return { ...state, loading: true, error: null };
    case 'DISCOVERY_COMPLETE':
      return { ...state, loading: false, boards: action.boards.sort(sortBoards), lastRefresh: new Date() };
    case 'DISCOVERY_ERROR':
      return { ...state, loading: false, error: action.error };
    case 'REFRESH_START':
      return { ...state, loading: true };
    case 'REFRESH_COMPLETE':
      return { ...state, loading: false, boards: action.boards.sort(sortBoards), lastRefresh: action.lastRefresh || state.lastRefresh };
    case 'BOARD_UPDATE':
      return {
        ...state,
        boards: state.boards.map(b => (b.id === action.board.id ? action.board : b)).sort(sortBoards),
      };
    case 'ADD_BOARD':
      if (state.boards.some(b => b.id === action.board.id)) return state;
      return { ...state, boards: [...state.boards, action.board].sort(sortBoards) };
    case 'REMOVE_BOARD':
